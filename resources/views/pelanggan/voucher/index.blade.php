...
@section('content')
    <div class="mb-6 flex justify-between items-center">
        ...
        <div class="flex gap-2">
            <span class="w-3 h-3 rounded-full bg-white border border-gray-300 inline-block mt-1"></span> <span
                class="text-xs text-gray-500">Aktif</span>
            <span class="w-3 h-3 rounded-full bg-gray-100 border border-gray-200 inline-block mt-1 ml-3"></span> <span
                class="text-xs text-gray-500">Terpakai</span>
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
        @forelse($vouchers as $v)
            <a href="{{ route('pelanggan.voucher.show', $v) }}"
                class="block relative bg-white border {{ $v->status === 'aktif' ? 'border-gray-200 hover:border-gray-400' : 'border-gray-100 opacity-60' }} rounded-xl overflow-hidden transition-all shadow-sm group">
                {{-- Header --}}
                <div
                    class="px-4 py-3 border-b {{ $v->status === 'aktif' ? 'border-gray-100 bg-gray-50' : 'border-gray-50 bg-gray-50/50' }}">
                    ...
                    <div class="w-24 h-24 relative">
                        ...
                        @if ($v->status === 'terpakai')
                            <div class="absolute inset-0 flex items-center justify-center bg-white/70">
                                <div
                                    class="transform -rotate-12 border-2 border-red-500 text-red-500 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded">
                                    Terpakai
                                </div>
                            </div>
                        @endif
                ...
                <div class="px-4 py-2 bg-gray-50 border-t border-gray-100 text-center">
                    ...
                    @else
                        <span class="text-[10px] font-medium text-gray-500 uppercase tracking-wider">Digunakan
                            {{ $v->usages->last()?->tgl_digunakan?->format('d/m/Y') }}</span>
                    @endif
                </div>
            </a>
        @empty
