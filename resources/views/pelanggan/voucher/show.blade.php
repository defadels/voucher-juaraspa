    ...
    <div class="max-w-md mx-auto relative">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden relative">
            ...
            <div class="p-8 text-center relative">
                @if ($voucher->status === 'terpakai')
                    <div class="absolute inset-0 z-10 flex items-center justify-center bg-white/80 backdrop-blur-[1px]">
                        <div
                            class="transform -rotate-12 border-4 border-red-500 text-red-500 text-2xl font-bold uppercase tracking-widest px-6 py-2 rounded-lg shadow-sm">
                            Sudah Terpakai
                        </div>
                    </div>
                @endif
                ...
                <div class="inline-block p-4 border-2 border-gray-100 rounded-2xl mb-5 bg-white shadow-sm">
                    <div class="w-48 h-48">
                        <img src="{{ asset('storage/' . $voucher->qr_code_path) }}" alt="QR Code"
                ...
                <div class="grid grid-cols-2 gap-4 text-left border-t border-gray-100 pt-6">
                    ...
                    <div>
                        ...
                        <p class="text-sm font-medium text-gray-900">
                            {{ \Carbon\Carbon::parse($voucher->tgl_terbit)->format('d M Y') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gray-50 border-t border-gray-100 px-6 py-4 text-center">
                ...
                @else
                    <p class="text-xs text-gray-500">Digunakan pada: <span
                            class="font-medium">{{ $voucher->usages->last()?->tgl_digunakan?->format('d/m/Y') }}</span></p>
                @endif
            </div>
        </div>
    </div>
