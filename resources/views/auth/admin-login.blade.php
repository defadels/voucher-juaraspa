...
<body class="font-sans bg-[#f5f5f0] flex h-screen">
    ...
    <aside class="w-[250px] min-w-[250px] bg-[#1a1a1a] text-white flex flex-col justify-between py-8 px-6">
        <div>
            ...
            <div class="space-y-5">
                @foreach([
                    ['icon' => 'lock', 'title' => 'Akses Aman', 'desc' => 'Login khusus admin terpisah dari pelanggan'],
                    ['icon' => 'qr', 'title' => 'Kelola Voucher', 'desc' => 'Terbitkan & pantau QR Code voucher pelanggan'],
                    ['icon' => 'chart', 'title' => 'Laporan Lengkap', 'desc' => 'Monitor pemakaian voucher secara real-time'],
                ] as $item)
                <div class="flex items-start gap-3">
                    <div class="w-8 h-8 rounded-lg bg-white/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                        @if($item['icon'] === 'lock')
                        <svg class="w-4 h-4 text-white/70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
                        @elseif($item['icon'] === 'qr')
    ...
    <main class="flex-1 flex items-center justify-center p-12">
        <div class="w-full max-w-md">
            ...
            <form method="POST" action="{{ route('admin.login') }}" class="space-y-5">
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-gray-500 mb-1.5" for="email">
                        Email Admin
                    </label>
                    <input
                        id="email"
                        ...
                        name="email"
                        value="{{ old('email') }}"
                        placeholder="admin@juaraspa.com"
                        required
                        autofocus
            ...
            <p class="mt-5 text-center text-sm text-gray-500">
                Pelanggan?
                <a href="{{ route('pelanggan.login') }}" class="underline text-gray-700 hover:text-gray-900">
